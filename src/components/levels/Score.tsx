...
export function ScoreLevel({
  ...
  const previous = commits[viewing + 1] ?? null;
  const scores = head ? score.scores : (commit?.scores ?? score.scores);
  ...
  return (
    <div>
      ...
      {!head && commit ? (
        <div className="scope">
          viewing <b>{commit.short}</b> · {commit.subject}
          <button onClick={onBackToHead}>back to head</button>
        </div>
      ) : (
        <p className="lede">
          Every category starts at 100 and is degraded only by a named rule with a published
          weight and a cap. Each section below says what was measured, why it counts, and what
          to do about it.
        </p>
      )}
      ...
      <div className="rings">
        {Object.entries(scores).map(([category, value]) => (
          <Ring
            ...
            category={category}
            value={value}
            delta={previous ? value - (previous.scores[category] ?? value) : null}
      ...
      {head ? (
        Object.entries(scores).map(([category, value]) => {
          ...
          return (
            <section className="cat" id={`cat-${category}`} key={category}>
              <div className="ch">
                <span className="cs" style={{ color: `var(--${band(value)})` }}>{value}</span>
                <h3>{category[0]?.toUpperCase()}{category.slice(1)}</h3>
                <span className="cl">
                  100 {lost > 0 ? `− ${lost.toFixed(1)}` : ''} ·{' '}
                  {rules.filter((r) => r.deducted > 0).length} of {rules.length} rules degraded
                </span>
              </div>
                  ...
                  <div className={`rule ${hit ? 'hit' : 'ok'}`} key={rule.rule}>
                    <div className="rh">
                      ...
                      <span className="rv">
                        ...
                        {cfg ? ` · free below ${cfg.free} · full at ${cfg.full}` : ''} · max {rule.weight}
                      </span>
                    ...
                    {hit && rule.remedy ? <p className="rx">{rule.remedy}</p> : null}
                    {rule.findings.length > 0 ? (
                      <details>
                        <summary>
                          ...
                          {' · '}
                          <span onClick={(e) => { e.preventDefault(); onJump(rule.level); }}>
                            {rule.level} →
                          </span>
                        </summary>
                        <div className="flist">
                          {rule.findings.map((f, i) => (
                            <div key={i}>
                              <span className="f">{f.file}{f.line ? `:${f.line}` : ''}</span>
                          ...
                          {rule.total_findings > rule.findings.length ? (
                            <div>
                              ...
                              <span className="w">
                                {(rule.total_findings - rule.findings.length).toLocaleString()} more
                              </span>
                            </div>
                          ) : null}
                        </div>
        ...
        <div className="empty">
          Per-rule detail is only kept for head. The walk records a score per category per
          commit; retaining every commit's findings needs <code>cqx history --keep-facts</code>.
        </div>
      )}

      <div className="legend">
        <span><i className="f" />0–49</span>
        <span><i className="w" />50–89</span>
        <span><i className="p" />90–100</span>
      </div>
    </div>
  );
}
